...
contract ArtGobblers is GobblersERC721, LogisticVRGDA, Owned, ERC1155TokenReceiver {
    ...
    /// @notice The address of a randomness provider. This provider will initially be a wrapper around chainlink VRF v1, but can be changed in case it is fully sunset.
    ...
    /// @notice Provenance hash for gobblers metadata.
    ...
    /// @dev We add 1 to LEGENDARY_SUPPLY because legendary auctions being only after the first interval.
    ...
    /// @notice Data about the current state of goobler reveal
    ...
    /// @notice Sets VRGDA parameters, mint config, relevant address and URIs.
    ...
    /// @param _baseUri Base URI for the revealed gobblers.
    /// @param _unrevealedUri URI for the unrevealed gobblers.
    ...
    constructor(
        ...
        LogisticVRGDA(
            ...
            // Max gobbler mintable via VRGDA.
    ...
    /// @dev Function does not directly enforce the MINTLIST_SUPPLY limit for gas efficiency. The limit is enforced
    ...
    function claimGobbler(bytes32[] calldata proof) external returns (uint256 gobblerId) {
        ...
        // Marked the user off the mintlist
    ...
    /// @notice Mint a goobler paying with goo.
    /// @param maxPrice Maximum price to pay to mint the gobbler.
    /// @param useVirtualBalance Whether the cost is paid from the user's virtual goo balance, or from their
    /// ERC20 balance.
    ...
    function mintFromGoo(uint256 maxPrice, bool useVirtualBalance) external returns (uint256 gobblerId) {
        ...
        // gobblerPrice() will revert once we reach it due to its logistic nature.
        // It will also revert prior to mint start.
        uint256 currentPrice = gobblerPrice();

        // If the current price is above the user's specified max, revert.
        if (currentPrice > maxPrice) revert PriceExceededMax(currentPrice);

        // Decrement the user's goo balance by the current price, either from virtual balance or erc20 balance.
        useVirtualBalance
            ? updateUserGooBalance(msg.sender, currentPrice, GooBalanceUpdateType.DECREASE)
            : goo.burnForGobblers(msg.sender, currentPrice);

        unchecked {
            ++numMintedFromGoo; // Overflow should impossible due to the supply cap.
    ...
    /// @dev Will revert if called before minting starts or after all gobbblers have been minted via VRGDA.
    /// @return Current price of a gobbler in terms of goo.
    function gobblerPrice() public view returns (uint256) {
        // We need checked math here to cause underflow before minting has begun, preventing mints.
    ...
    function mintLegendaryGobbler(uint256[] calldata gobblerIds) external returns (uint256 gobblerId) {
        ...
        // This will srevert if the auction hasn't started yet or legendaries have sold out entirely, so there's no need to check here as well.
        ...
        unchecked {
            uint256 burnedMultipleTotal; // The legendary's emissionsMultiple will be 2x the sum of the gobblers burned.

            //BATCH BURNE LOGIC

            uint256 id; // storing the loop saves `7 gas iteration.
            ...
            // The legendary's emmissionsMultiple is 2x the sum of the multiples of the gobblers burned.

            getGobblerData[gobblerId].emissionMultiple = uint32(burnedMultipleTotal * 2);

            // Update the user's data struct in one big batch. We add burnedMultipleTotal
            // to their emmission multiple (not burnedMultipleTotal * 2) to account for
            // the standard gobblers that were burned and hence should have their
            // multiples subtracted from the user's total multiple
            getUserData[msg.sender].lastBalance = uint128(gooBalance(msg.sender));
            // checkpoint balance.
            ...
            // Update the total number of gobblers owned by the user. The call to _mint below will increase the count by 1 to account for the new legendary gobbler.
