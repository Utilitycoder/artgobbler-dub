...
contract Pages is PagesERC721, LogisticToLinearVRGDA {
    ...
    /// @notice the addres of the goo ERC20 token contract.
    Goo public immutable goo;

    /// @notice The address of which receives pages reserved for the community
    ...
    /// @dev Will be 0 if no pages has been minted yet.
    ...
    /// @notice The minimum amount of pages that must be sold for the VRGDA issuance schedule to switch from logistic to the "post switch" translated linear formula
    ...
    ) PagesERC721(_artGobblers, "Pages", "PAGE") LogisticToLinearVRGDA(4.2069e18, // Target price
    ...
    /// @param useVirtualBalance Whether the the cost is paid from the user's virtual goo balance, or from their ERC20 goo balance.
    ...
    /// @notice Calculate tje mint cost of a page.
    /// @dev If the number of sales is below a predetermined threshold , we use the VRGDA pricing algorithm, otherwise we use the post-switch pricing formula.
    /// @dev Reverts due to underflow if mintng hasn't started yet. Done to save gas.
    function pagePrice() public view returns (uint256) {
        ...
        unchecked {
            // The number of pages minted for the community reserve should never exceed 10% of the total supply of the pages.
    ...
    /// @notice Mint a number of the pages to the community reserve.
    /// @param numPages The number of pages to mint to the reserve.
    /// @dev Pages minted to the reserve cannot comprie more than 10% of the sum of the supply goo minted pages and the supply of pages minted to the community reserve.
    function mintCommunityPages(uint256 numPages) external returns (uint256 lastMintedPageId) {
        unchecked {
            ...
            // Overflow in this calculation is possible but numPages would have to be so large that it would cause the loop in _batchMint to run out of gas quickly. 
            uint256 newNumMintedForCommunity = numMintedForCommunity += uint128(numPages);

            // Ensure that after this mint pages minted to the community reserve won't comprise more than 10% of the new total page supply. CurrentId is equivalent to the current total supply of pages.
